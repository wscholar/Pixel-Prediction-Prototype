...
public class ExperimentHarness {
	...
	private int flc_timestep_modulus;

	//TODO: setup logging
	//TODO setup output formatting options
	...
	{	
		...
		this.double_ended = double_ended;
		this.flc_timestep_modulus = double_ended ? this.h_p + 1 : 1;
	}
	...
	private double[] add_buffer_frame(double[] raveled_data, int[] data_geometry) {
		int num_buffer_elements = Utils.product(Utils.slice(data_geometry, 1, data_geometry.length));
		double[] out = new double[raveled_data.length + num_buffer_elements];
		for (int i = num_buffer_elements; i < out.length; i++) {
			out[i] = raveled_data[i - num_buffer_elements];
		}
		return out;
	}
	...
	{
		if (this.lce == null) {
			this.lce = new LightConeExtractor(this.speed, this.h_p, this.rescale_data, this.double_ended);
		}
	...
	public LightConesSet train_test_split(double[] raveled_data, int[] data_geometry, int test_start, int test_end) 
	{
	...
	public void exp_trial(String data_dir, String results_root_dir, String results_suffix, int trial, int start_fold, 
		...
		{						
			...
			lcSet = train_test_split(data, this.dataGeometry, start, stop);
